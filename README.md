...
**Current version (1.0.0) is the Foundation release**: a solid, modular base with browsing of all calls plus per-call recording playback/download. More features (queue reports, live AMI tools, scheduled reports, IVR analytics, and more) are built on top of this base.

## Features

- **All Calls** — browse CDR records with filters (date range, source, destination, caller ID, status), pagination, and **CSV export**
- **Call recordings** — play (inline) or **download** the recorded audio for each call (WAV/MP3/OGG/GSM), served through an authenticated endpoint
- **Dashboard** — today's summary: total / answered / not-answered calls and average talk time, plus recent calls
