...
function ask(string $label, string $default = '', bool $required = false): string
{
...
if (!version_compare(PHP_VERSION, '7.4.0', '>=')) {
    fwrite(STDERR, 'PHP 7.4 or newer required (found ' . PHP_VERSION . ').' . PHP_EOL);
    exit(1);
}
$missingExt = [];
...
$db = new PDO('mysql:host=localhost;port=3306;dbname=' . $dbName . ';charset=utf8mb4', $dbUser, $dbPass, [
...
if ($rootExists === 0) {
    $db->prepare('INSERT INTO smr_users (username, password_hash, role, display_name, active, created_at) VALUES (?, ?, ?, ?, 1, NOW())')
        ->execute([$rootUser, password_hash($rootPass, PASSWORD_DEFAULT), 'root', 'Module Owner']);
...
if ($adminExists === 0) {
    $db->prepare('INSERT INTO smr_users (username, password_hash, role, display_name, active, created_at) VALUES (?, ?, ?, ?, 1, NOW())')
        ->execute([$adminUser, password_hash($adminPass, PASSWORD_DEFAULT), 'admin', 'Administrator']);
...
out('');
out(' Web panel:  http://YOUR-SERVER/' . basename(SMR_ROOT) . '/');
