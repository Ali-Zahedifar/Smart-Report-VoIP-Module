...
final class Auth
{
    ...
    public static function attempt($username, $password)
    {
        ...
        if ($user !== null && smr_password_verify($password, $user['password_hash'])) {
            self::resetAttempts();
            self::login($user);
            return true;
        }
    ...
    public static function recordFailure()
    {
        ...
        $fails = (int) (isset($_SESSION['_login_fails']) ? $_SESSION['_login_fails'] : 0);
        $fails++;
    ...
    public static function lockedSeconds()
    {
        if (SMR_CLI) {
            return 0;
        }
        $until = (int) (isset($_SESSION['_login_locked_until']) ? $_SESSION['_login_locked_until'] : 0);
        if ($until <= 0) {
            return 0;
        }
